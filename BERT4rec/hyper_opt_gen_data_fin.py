...
def parse_args():
    ...
    parser.add_argument("--dataset_name", default='rsc15', help="dataset name.", type=str)
...
def split_train_kFold(n_folds, dataset, output_dir_name):
    kfold = KFold(n_splits=n_folds, shuffle=True)
    for train, test in kfold.split(dataset):
        train_data = {index_train: dataset[index_train] for index_train in train}
        test_data = {index_test: dataset[index_test] for index_test in test}
...
def main():
    ...
    for train, test in kfold.split(user_train_data):
        ...
        train_output_file_name = kFoldOutputDir + dataset_name + version_id + fold_id + ".train.tfrecord"
        gen_samples(train_data, train_output_file_name, rng, vocab, max_seq_length, dupe_factor, mask_prob,
                    masked_lm_prob, max_predictions_per_seq, prop_sliding_window, pool_size, force_last=False)
        test_output_file_name = kFoldOutputDir + dataset_name + version_id + fold_id + ".test.tfrecord"
        gen_samples(test_data, test_output_file_name, rng, vocab, max_seq_length, dupe_factor, mask_prob,
                    masked_lm_prob, max_predictions_per_seq, -1.0, pool_size, force_last=True)
        ...
        vocab_file_name = kFoldOutputDir + dataset_name + version_id + fold_id  + '.vocab'
        ...
        his_file_name = kFoldOutputDir + dataset_name + version_id + fold_id + '.his'
